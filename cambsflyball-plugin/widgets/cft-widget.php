...
class cft_widget extends WP_Widget {
    ...
    public function widget( $args, $instance ) {
        ...
        from seed_list where team_name like 'Cambridgeshire%' and is_mb=0 order by seed_position";
        ...
        if (count($results) > 0){
            ...
            foreach ($results as $row){
                echo '<tr>
                <td width="25%" class="has-text-align-center" data-align="center">'.$row->seed_position.getOrdinal($row->seed_position).'</td>
                <td><a href="/teams/'.$row->slug.'/" data-type="URL">'.$row->team.'</a></td>
                <td width="30%" class="has-text-align-center" data-align="center">'.number_format($row->seed_time, 2).'</td></tr>';
            }
            echo '</tbody></table></figure>';    
        }
        //Mulit-breed
        $sql = "select seed_position, seed_time, race_date, is_mb, replace(lower(team_name), ' ', '-') as slug, replace(team_name, 'Cambridgeshire ', '') as team
        from seed_list where team_name like 'Cambridgeshire%' and is_mb=1 order by seed_position";
        $results = $wpdb->get_results($sql);
